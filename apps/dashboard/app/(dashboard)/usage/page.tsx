import { ChartBar, ChartLineUp, PlugsConnected } from "@phosphor-icons/react/dist/ssr";
...
export default async function UsagePage({
  ...
  if (scope.activeOrg === null) {
    return (
      <>
        <PageHeader title="Usage" description="Usage analytics across your organization." />
        <EmptyState
          icon={ChartBar}
  ...
  const series = hasAccount ? unwrapOr(await fetchUsageByDay(account, start, end), []) : [];
                ...
                <TableBody>
                  {byModel.map((usage) => (
                    <TableRow key={usage.model}>
                      <TableCell className="font-mono text-xs">{usage.model}</TableCell>
