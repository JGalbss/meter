...
const DELIVERY_VARIANTS = { delivered: "secondary", failed: "destructive" } as const;
            ...
            <TableBody>
              ...
              {endpoints.map((webhook) => (
                <TableRow key={webhook.id}>
                  <TableCell className="max-w-80 truncate font-mono text-xs">{webhook.url}</TableCell>
                  <TableCell className="text-muted-foreground">
                    {eventsLabel(webhook.eventTypes)}
                  </TableCell>
                  <TableCell>
                    <ValueBadge value={enabledLabel(webhook.enabled)} variants={ENABLED_VARIANTS} />
                  </TableCell>
            ...
            <TableBody>
              ...
              {log.map((delivery) => (
                <TableRow key={delivery.id}>
                  ...
                  <TableCell>
                    <ValueBadge value={delivery.status} variants={DELIVERY_VARIANTS} />
                  </TableCell>
                  <TableCell className="tabular-nums">{dash(delivery.responseStatus)}</TableCell>
