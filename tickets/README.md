...
- [~] First-class per-client auto-patch wrappers (monkey-patch a provider client) — **TS done**:
  `patchAnthropic`/`patchOpenAI` wrap a duck-typed provider client's `create` method so every call is
  metered automatically (charge or record mode), generating a per-call idempotency key, deriving the
  model from the response (then the request), fail-open via `onError`, and returning the provider
  response unchanged; each returns an `Unpatch` to restore the original. No dependency on the provider
  packages (structural typing). Tested (7 tests). Python parity pending.
