...
- [~] **Dependency audit + CI gate.** The **runtime** advisory is fixed: drizzle-orm 0.38→**0.45.2**
  (+ drizzle-kit 0.31.10) — typecheck, all 27 control-plane tests, and `db:generate` (no schema drift)
  green; advisories 8→6. The remaining 6 are **dev-only** tooling (vitest critical → 3.2.6+, vite/esbuild
  via vitest) needing a vitest 2→3 major bump across the control-plane + SDK suites. Add a
  `pnpm audit --audit-level high` + `cargo audit` CI gate once the dev chain is bumped.
