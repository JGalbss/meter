...
- [ ] Private-VPC notes; `meter migrate` job option; expose control-plane/engine via Ingress paths
...
- [ ] **Dependency audit + CI gate.** `pnpm audit` currently flags 8 advisories, all in TS tooling/ORM:
  vitest (critical, dev) → 3.2.6+, vite/esbuild (dev, transitive via vitest), drizzle-orm (high,
  runtime) → 0.45.2+. Fixing needs deliberate **major bumps** (vitest 2→3 config changes; drizzle-orm
  0.38→0.45 query/migrator changes) verified against the full control-plane + SDK suites — not a rushed
  bump on the shared branch. Add a `pnpm audit --audit-level high` + `cargo audit` CI gate once green.
